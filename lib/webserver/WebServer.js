...
const WebServer = function (options) {
	...
	return new Promise(function(resolve, reject) {
	    if (broadcastRunning) {
            	nextBroadcast = data;
		reject("A broadcast is already running. Only running one broadcast at a time.");
	    } else {
		nextBroadcast = null;
                return Promise.all(wss.clients.map(function each(ws) {
                    return new Promise(function(resolve, reject) {
                        ws.send(mapJSON, resolve);
                    });
                })).then(function() {
                    if (nextBroadast) {
